...
def eval_and_label_dataset(dataloader, model, banks, args):
	...
	# iterator = dataloader if is_master(args) else dataloader
	...
	all_probs = nn.Softmax(dim=1)(logits)
	...
	if args.distributed:
		...
		all_probs = concat_all_gather(all_probs)
		...
		all_probs = remove_wrap_arounds(all_probs, ranks)
	...
	nll = nn.NLLLoss()(nn.LogSoftmax(dim=1)(logits), gt_labels)
	...
	log_str = f"Accuracy of direct prediction: {accuracy:.2f}"
	...
	if args.data.dataset == "VISDA-C":
		...
		wandb_dict["Test Ent"] = ent
		wandb_dict["Test NLL"] = nll
		wandb_dict["Test BSC"] = bsc
		wandb_dict["Test ECE"] = ece
		wandb_dict["Test MCE"] = mce
...
def get_augmentation_versions(args):
	...
	for version in args.learn.aug_versions:
		if version == "s":
			...
			transform_list.append(get_augmentation(args.data.aug_type, pasta_args=pasta_args))
		elif version == "w":
...
def get_bn_only_optimizer(model, args):
	if args.distributed:
		model = model.module
	bn_params = (
		model.src_model.get_bn_params()
		if hasattr(model, "src_model")
		else model.get_bn_params()
	)
...
def train_target_domain(args):
	...
	if args.learn.bn_only:
		print("Trying to set up optimizer..")
		optimizer = get_bn_only_optimizer(model, args)
	else:
...
def train_epoch(train_loader, model, banks, optimizer, epoch, args):
	...
	top1_ref_psd = AverageMeter("CLS-PSD-GT-Acc@1", ":6.2f")
	progress = ProgressMeter(
		len(train_loader),
		[batch_time, loss_meter, top1_ins, top1_psd, top1_ref_psd],
	...
	for i, data in enumerate(train_loader):
		...
		with torch.no_grad():
			...
			pseudo_labels_w, probs_w, ref_acc = refine_predictions(
				feats_w, probs_w, banks, args=args, gt_labels=img_labels
			)

		top1_ref_psd.update(ref_acc.item(), len(logits_w))
		...
		if args.learn.ent_min:
			loss_cls += args.learn.ent_coeff * entropy_minimization(logits_w)
...
def subset_cross_entropy_loss(logits, labels, args):
    conf_vals = F.softmax(logits, dim=1)
    ent_vals = utils.Entropy(conf_vals.clone())
    conf_vals, _ = conf_vals.max(dim=1)
    masks = (conf_vals > args.learn.pl_thresh).float()
